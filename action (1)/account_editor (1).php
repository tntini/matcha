<?php
	...
	if (isset($_POST['subusername']))
	{
		if (empty($_POST['username']))
			put_flash('danger', "Empty username.", "../account.php");

		if (!preg_match('/^[a-zA-Z0-9]+$/', $_POST['username']) || strlen($_POST['username']) > 20)
			put_flash('danger', "Error : Invalid username. a-zA-Z0-9 < 20 char", "../account.php");
		...
		if ($exi)
			put_flash('danger', "Username already taken.", "../account.php");
		...
		put_flash('success', "Username modified.", "../account.php");
	}
	...
	if (isset($_POST['subpsw']))
	{
		if ($_POST['psw'] !== $_POST['pswr'] || strlen($_POST['psw']) > 20)
			put_flash('danger', "Error : Invalid password", "../account.php");
		...
		put_flash('success', "Password modified.", "../account.php");
	}
